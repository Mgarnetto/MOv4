...
function renderVaultGrid(posts, isOwner) {
    ...
    posts.forEach(post => {
        ...
        const safeDesc = encodeURIComponent(post.text || '').replace(/'/g, "%27");
        const safeVidUrl = encodeURIComponent(vidUrl).replace(/'/g, "%27");
...
function renderCollectionsGrid(collections, isOwner) {
    ...
    visibleCollections.forEach(col => {
        ...
        const safeTitle = encodeURIComponent(title).replace(/'/g, "%27");
        const safeDesc = encodeURIComponent(col.description || '').replace(/'/g, "%27");
                ...
                <div class="vid-thumb-wrapper" onclick="window.viewVideoCollection('${col.id}', '${safeTitle}')">
                    ...
                    <div class="vid-top-right-actions">
                        ...
                        <div class="msg-context-menu" style="position: absolute; right: 0; top: 100%; width: 170px; z-index: 1050; background: #222; border: 1px solid #444; border-radius: 6px; display: none;">
                            <button onclick="event.stopPropagation(); window.openVideoInspector('${col.id}', 0, '${safeTitle}', '${safeDesc}', ${price}, ${vis}, '${art}', 0)" style="background: transparent; border: none; padding: 10px 15px; width: 100%; text-align: left; color: #fff; cursor: pointer; display: flex; align-items: center; gap: 10px;"><i class="fas fa-edit"></i> Edit Collection</button>
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (items.length === 0) {
                ...
                items.forEach((item) => {
                    const safeTitle = encodeURIComponent(item.title || item.Title).replace(/'/g, "%27");
                    const safeUrl = encodeURIComponent(item.url || item.Url).replace(/'/g, "%27");
...
// ============================================
// 4. VIDEO INSPECTOR (WITH DUAL IDS)
// ============================================
...
window.openVideoInspector = async function (mediaId, postId, titleEnc, descEnc, price, visibility, coverUrl, targetType = 2) {
    ...
    // Store Both IDs cleanly
    document.getElementById('vid-edit-target-id').value = mediaId;
    let postInput = document.getElementById('vid-edit-post-id');
    if (!postInput) {
        postInput = document.createElement('input');
        postInput.type = 'hidden';
        postInput.id = 'vid-edit-post-id';
    ...
    document.getElementById('vid-edit-price').value = price > 0 ? parseFloat(price).toFixed(2) : '';
    ...
    if (targetType === 0 || targetType === '0') {
        ...
        window.loadInspectorEpisodeList(mediaId); // acts as collectionId here
    } else {
...
window.saveVideoInspector = async function () {
    ...
    // FIX: Pull both IDs accurately
    const mediaId = document.getElementById('vid-edit-target-id').value;
    const postId = document.getElementById('vid-edit-post-id')?.value || 0;
    ...
    try {
        ...
        // FIX: Construct the full Dual-ID Payload
        const payload = {
        ...
        const endpoint = (type == 0 || type == '0') ? `/api/videohub/collection/${mediaId}` : `/api/videohub/video/${mediaId}`;
